...
class SODSegDataPreProcessor(BaseDataPreprocessor):
    ...
    def __init__(
        ...
        if mean_ir is not None:
            assert std_ir is not None, 'To enable the normalization in ' \
                                    'preprocessing, please specify both ' \
                                    '`mean_ir` and `std_ir`.'
            # Enable the normalization in preprocessing.
            self._enable_normalize = True
    ...
    def forward(self, data: dict, training: bool = False) -> Dict[str, Any]:
        ...
        # XXX: map all the data to cuda
        data = self.cast_data(data)  # type: ignore
        inputs = data['inputs']  # inputs contains the inputs X of a minibatch
        data_samples = data.get('data_samples', None)
        # TODO: whether normalize should be after stack_batch
        if self.channel_conversion and inputs[0].size(0) == 6:
            # vis + ir concated, chang vis and ir from bgr to rgb
            inputs = [_input[[2, 1, 0, 5, 4, 3], ...] for _input in inputs]

        inputs = [_input.float() for _input in inputs]
        if self._enable_normalize:  # NOTE: do normalization together
            mean = torch.cat((self.mean, self.mean_ir), dim=0)
            std = torch.cat((self.std, self.std_ir), dim=0)
