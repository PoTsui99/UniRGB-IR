...
class Attention(nn.Module):
    ...
    def forward(self, x):
        B, H, W, _ = x.shape  # 输入的是 B 个 3D tensor(没有被 flatten)
...
class ConvMixFusion(nn.Module):
    def __init__(self, kernels=(3, 3, 5, 7), groups=4, inplanes=64):
        ...
        # 计算量还好, 就不进行 bottleneck, channel_per_group=16
...
class StageBlock(nn.Module):  # Includes Transformer Blocks' Forward
    def __init__(self, dim, n_levels, num_heads=6, n_points=4, deform_ratio=1.0,
                 ...
                 first_stage=False):  
        super().__init__()
        self.first_stage = first_stage  # 第一个 stage 用不到
        ...
        # if not first_stage:
        #     self.gamma = nn.Parameter(init_values * torch.ones(dim), requires_grad=True)  # NOTE: 需要斟酌
...
class ViTRGBTv15(BaseModule):
    ...
    def forward(self, inputs):
        vis = inputs[:, :3, :, :]
        ir = inputs[:, 3:, :, :]
        ir = ir.repeat([1, 3, 1, 1])  # 1 channel to 3 channel
        ...
        for i, stage in enumerate(self.adapter_stages):
            ...
            outs.append(x.transpose(1, 2).view(B, dim, H, W).contiguous()) # NOTE: output after each stage
        return tuple(outs)
