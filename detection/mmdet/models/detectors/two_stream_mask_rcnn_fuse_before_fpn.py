...
# 分别使用两个 backbone 去获得 Vis 和 IR 特征, 整合后输入特征金字塔
# 基于 Mask R-CNN, 和 RetinaNet 整体结构不同
class TwoStreamTwoStageDetectorFuseBeforeFPN(BaseDetector):  
    ...
    def extract_feat(self, batch_inputs: Tensor) -> Tuple[Tensor]:
        ...
        # call the forward() of backbone module
        x = self.backbone(batch_inputs['img'], batch_inputs['img_ir'])  # TODO: add loss factor here when training?
        if self.with_neck:
            x = self.neck(x)
        return x
