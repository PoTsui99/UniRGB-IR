...
def ncc(feature1, feature2):
    feature1_mean = np.mean(feature1)
    feature2_mean = np.mean(feature2)
    numerator = np.sum((feature1 - feature1_mean) * (feature2 - feature2_mean))
    denominator = np.sqrt(np.sum((feature1 - feature1_mean)**2) * np.sum((feature2 - feature2_mean)**2))
...
class TwoStreamTwoStageDetectorVisualizer(BaseDetector):  
    ...
    def predict(self,
        ...
        save_root = Path.cwd() / 'work_dirs' / 'vit_nccl'
        ...
        vit_cnn_ncc_file = open(save_root / 'vit-cnn_ncc.txt', 'a')
        vit_prompt_ncc_file = open(save_root / 'vit-prompt_ncc.txt', 'a')
        ...
        for i, sample in enumerate(batch_data_samples):  # NOTE: for each sample in this batch
            ...
            vit_input_np = vit_input.cpu().numpy()
            cnn_input_np = cnn_input_viz.cpu().numpy()
            prompt_np = prompt.cpu().numpy()
            
            vit_cnn_ncc = ncc(vit_input_np, cnn_input_np)
            vit_prompt_ncc = ncc(vit_input_np, prompt_np)
            
            vit_cnn_ncc_file.write(f'{vit_cnn_ncc}\n')
            vit_prompt_ncc_file.write(f'{vit_prompt_ncc}\n')
            
            vit_cnn_ncc_list.append(vit_cnn_ncc)
            vit_prompt_ncc_list.append(vit_prompt_ncc)
        ...
        vit_cnn_ncc_file.close()
        vit_prompt_ncc_file.close()
        
        vit_cnn_avg_ncc = sum(vit_cnn_ncc_list) / len(vit_cnn_ncc_list)
        vit_prompt_avg_ncc = sum(vit_prompt_ncc_list) / len(vit_prompt_ncc_list)
        
        print(f'Batch-Average NCC (vit-cnn): {vit_cnn_avg_ncc}')
        print(f'Batch-Average NCC (vit-prompt): {vit_prompt_avg_ncc}')
