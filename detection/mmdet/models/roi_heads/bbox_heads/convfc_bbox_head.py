    ...
    def __init__(self,
        ...
        if self.with_cls:
            ...
            cls_predictor_cfg_.update(
                in_features=self.cls_last_dim, out_features=cls_channels)  # 输出通道变为 前景类别数+1
            self.fc_cls = MODELS.build(cls_predictor_cfg_)
        if self.with_reg:
            box_dim = self.bbox_coder.encode_size
            out_dim_reg = box_dim if self.reg_class_agnostic else \
                box_dim * self.num_classes  # 默认是对每个类别输出 bbox_dim (reg_class_agnostice=False), 所以 num_classes 不考虑背景 (背景bbox咋能reg?)
