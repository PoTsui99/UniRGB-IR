...
# by tsuipo 23.9.14
@TRANSFORMS.register_module()
class AlignedImagesResize(Resize):
    def _resize_img(self, results: dict) -> None:
        ...
        if results.get('img_ir', None) is not None:
            if self.keep_ratio:
                ...
                img_ir, w_scale, h_scale = mmcv.imresize(
                    results['img_ir'],
                    results['scale'],
                    interpolation=self.interpolation,
                    return_scale=True,
...
@TRANSFORMS.register_module()
class AlignedImagesRandomResize(MMCV_RandomResize):
    def __init__(
        self,
        scale: Union[Tuple[int, int], Sequence[Tuple[int, int]]],
        ratio_range: Tuple[float, float] = None,
        resize_type: str = 'AlignedImagesResize',  # 原来是 Resize, 相当于是 wrapper
        **resize_kwargs,
    ) -> None:
...
# by tsuipo 23.9.14
@TRANSFORMS.register_module()
class AlignedImagesRandomFlip(RandomFlip):
    """Flip the image & bbox & mask & segmentation map. Added or Updated keys:
    flip, flip_direction, img, gt_bboxes, and gt_seg_map. There are 3 flip
    modes:
...
class AlignedImagesPad(Pad):
    def _pad_img(self, results: dict) -> None:
        ...
        if isinstance(pad_val, int) and results['img_ir'].ndim == 3:
            pad_val = tuple(pad_val for _ in range(results['img_ir'].shape[2]))
        padded_img_ir = mmcv.impad(
            results['img_ir'],
            shape=size,
            pad_val=pad_val,
            padding_mode=self.padding_mode)   
...
@TRANSFORMS.register_module()
class AlignedImagesRandomCrop(RandomCrop):
    def _crop_data(self, results: dict, crop_size: Tuple[int, int],
                   allow_negative_crop: bool) -> Union[dict, None]:
        """Function to randomly crop images, bounding boxes, masks, semantic
        segmentation maps.
